...
func play_card_by_index(index: int, actor: PlayerData) -> void:
	...
	# Трата ресурсов
	match card.type:
		CardData.ResourceType.BRICKS: actor.bricks -= card.cost
		CardData.ResourceType.GEMS: actor.gems -= card.cost
		CardData.ResourceType.BEASTS: actor.beasts -= card.cost
	...
	for effect in card.effects:
		...
		match type:
			...
			"build_tower":
				...
				GameEvents.health_changed.emit(target_player, value)
			"mod_quarry": target_player.quarry += value
			"mod_magic": target_player.magic += value
			"mod_dungeon": target_player.dungeon += value
...
func check_win() -> bool:
	...
	if player_data.tower_hp >= WIN_TOWER_HEIGHT or enemy_data.tower_hp <= 0:
		...
		winner = enemy_data
	elif player_data.bricks >= WIN_RESOURCE_AMOUNT or player_data.gems >= WIN_RESOURCE_AMOUNT or player_data.beasts >= WIN_RESOURCE_AMOUNT:
		winner = player_data
	elif enemy_data.bricks >= WIN_RESOURCE_AMOUNT or enemy_data.gems >= WIN_RESOURCE_AMOUNT or enemy_data.beasts >= WIN_RESOURCE_AMOUNT:
		winner = enemy_data
