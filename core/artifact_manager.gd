...
func apply_artifact_effect(artifact: ArtifactData, effect: Dictionary, player: PlayerData) -> void:
	...
	match type:
		"mod_quarry": player.quarry += value
		"mod_magic": player.magic += value
		"mod_dungeon": player.dungeon += value
		"build_wall": player.wall_hp += value
		"build_tower": player.tower_hp += value
