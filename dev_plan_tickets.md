...
> ⚠️ Частично реализовано: `tests/fixtures.gd` — `TestFixtures` (`class_name`, без префикса `test_`,
...
> единая точка правды для дефолтных тестовых значений. Не проверено вживую из песочницы (нет Godot) —
> нужно прогнать GUT и убедиться, что все 25 тестов всё ещё зелёные после рефакторинга.
