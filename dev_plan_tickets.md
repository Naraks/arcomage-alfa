...
### ARC-081 — Наглядно показать генераторы ресурсов (свои и противника) в бою
- **Тип:** Task · **Приоритет:** Medium · **Оценка:** 2 SP

**Описание:** В бою должно быть хорошо видно текущую скорость прироста ресурсов (Каменоломня/Магия/Подземелье —
`quarry`/`magic`/`dungeon`) у обеих сторон, чтобы можно было сравнить экономику игрока и противника с одного
взгляда. Сейчас `battle_screen.gd` уже технически выводит эти значения — `update_all_ui()` формирует метки вида
`"Bricks: %d (+%d)"` для игрока и `"Bricks: %d (+%d)"` (аналогично Gems/Beasts) для противника, где `+%d` — это и
есть `quarry`/`magic`/`dungeon`. Но это мелкий суффикс внутри строки с текущим количеством ресурса, а не отдельный
наглядный блок «генераторы» — легко не заметить, особенно у противника. Нужно решить (на этапе реализации), как
сделать это заметнее: отдельный ряд/иконки генераторов рядом с ресурсами, или просто более крупный/контрастный
стиль для `(+N)`.

**Критерии приёмки:**
- [ ] Генераторы игрока (`quarry`/`magic`/`dungeon`) заметно отображаются в бою.
- [ ] Генераторы противника отображаются так же наглядно, как свои.
- [ ] Значения обновляются вживую при `resource_changed`/`turn_started` (как остальной UI боя).
