...
> ⚠️ Частично реализовано: `health_changed` полностью удалён из `game_events.gd`, заменён на
...
> перенесён внутрь каждой ветки. Не проверено вживую: тряска экрана при уроне и обновление UI при строительстве
> в редакторе Godot; существующие GUT-тесты (ни один не ссылается на `health_changed` напрямую, но нужно
> прогнать полный набор).
