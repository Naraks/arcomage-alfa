...
> ⚠️ Частично реализовано: настоящего line-coverage инструмента для GDScript/GUT нет (единственный
...
> в тестах, `match_manager.gd` — 57%. Новый джоб `coverage-report` в `.github/workflows/ci.yml`
> (`needs: tests`, только `python3`, Godot не нужен) прикладывает `coverage_report.txt` как artifact на
> каждый push/PR. Не проверено вживую в реальном CI из песочницы.
