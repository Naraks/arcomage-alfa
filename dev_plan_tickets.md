...
> ⚠️ Частично реализовано: `.github/workflows/ci.yml` шлёт сообщение в Telegram через Bot API (`curl`,
> без сторонних actions) в двух точках — падение джоба `tests` на push в `main` (обязательная
> проверка), и успешный деплой Web-сборки на GitHub Pages (см. ARC-070) как сигнал «новая сборка
> готова», это же покрывает вторую часть описания тикета. Оба шага тихо не выполняют ничего (без
> падения джоба), если секреты `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` не заведены. Не проверено
> вживую — нужно завести бота через @BotFather, получить `chat_id`, добавить оба значения в Settings →
> Secrets and variables → Actions, и один раз уронить/поднять CI, чтобы увидеть сообщения.
