...
> ⚠️ Частично реализовано: `.gitattributes` расширен LFS-правилами, инструкция по установке — в
> `CONTRIBUTING.md`. Не проверено фактическое поведение (нет доступа к бинарнику `git-lfs` в песочнице
> агента) — критерии приёмки останутся неотмеченными, пока кто-то не подтвердит вручную.
