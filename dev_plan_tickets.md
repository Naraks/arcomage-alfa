...
> ⚠️ Частично реализовано: `.github/PULL_REQUEST_TEMPLATE.md`, `.github/ISSUE_TEMPLATE/bug_report.md`
> и `config.yml` созданы, описаны в `CONTRIBUTING.md`. Не проверено — репозиторий ещё не запушен на
> GitHub из песочницы агента (сеть заблокирована), значит не подтверждено, что GitHub реально
> подставляет шаблоны при создании PR/issue. Нужно запушить `origin main` вручную и проверить.
