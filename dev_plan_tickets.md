...
> ⚠️ Частично реализовано: `.github/workflows/ci.yml` написан (checkout+LFS → скачать Godot 4.7.1 →
> прогреть импорт → `gut_cmdln.gd -gexit -gexit_on_success` → JUnit-отчёт как artifact). Не проверено —
> GitHub Actions недоступен из песочницы агента, нужно запушить и посмотреть первый прогон во вкладке
> Actions. После зелёного прогона зафиксировать его длительность как baseline (третий критерий) —
> просто длительность job, видна в самом интерфейсе Actions.
