...
> ⚠️ Частично реализовано: `MatchSettings` — новые поля `came_from_map: bool`, `current_map_node:
...
>
> Не проверено вживую из песочницы (нет Godot) — нужно пройти в редакторе: карта → бой → победа →
> «Вернуться на карту» → узел должен быть `disabled`; отдельно проверить путь через поражение и прямой
> тестовый бой из главного меню.
...
> ⚠️ Частично реализовано: `_test_setup()` теперь всегда создаёт свежие `PlayerData.new()` вместо
...
> создаёт `PlayerData.new()` на каждый клик по узлу, баг их не касался. Не проверено вживую из
> песочницы (нет Godot) — нужно пройти тестовый бой, проиграть, нажать Restart и убедиться, что башня и
> стена начинаются заново.
