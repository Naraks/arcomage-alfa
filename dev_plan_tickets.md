...
> ⚠️ Частично реализовано: `.pre-commit-config.yaml` (gdtoolkit `gdformat`/`gdlint`) и `.editorconfig`
> (`[*.gd]` — tab, размер 4) готовы, инструкция — в `CONTRIBUTING.md`. Не проверено — нет сети до
> PyPI/GitHub в песочнице агента, чтобы поставить `gdtoolkit` и прогнать `pre-commit run --all-files`
> по существующим 19 `.gd`-файлам. Критерии приёмки останутся неотмеченными, пока это не сделано
> вручную и не закоммичен отдельный «чистый» коммит форматирования.
