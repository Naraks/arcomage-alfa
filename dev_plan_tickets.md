...
- [x] `export_presets.cfg` возвращён к состоянию ARC-006 (`canvas_resize_policy=1`, Yandex SDK в
      `head_include`).
- [ ] Верхняя часть экрана боя не обрезается визуально — нужна проверка глазами в редакторе/экспорте.

> ⚠️ Частично реализовано: `battle_screen.gd` — `_update_visuals()` теперь ограничивает число блоков
...
> что-то в нём пересохранит. Не проверено вживую: сам факт того, что верх экрана больше не обрезается —
> нужен взгляд в редакторе/собранном Web-билде.
