...
файлов планирования (`arcomage_plan_deprecated.md`, `arcomage_roadmap_deprecated.md`, `arcomage_detailed_roadmap_deprecated.md`,
`arcomage_implementation_detail_deprecated.md`, `cards_list.md`, `checklist.md`). Часть пунктов в `checklist.md` устарела —
...
* **Два ИИ-профиля** (`default_ai_strategy.gd`, `aggressive_ai_strategy.gd`) с весовой системой оценки карт —
  реализованы, профили «Строитель» и «Маг/Экономист» из `arcomage_plan_deprecated.md` — ещё нет.
...
Рекомендация: заморозить старые roadmap-файлы как исторический контекст и вести работу по бэклогу ниже —
он синхронизирован с реальным кодом.
...
(«Проклятие»), условными эффектами («Резервы»: если стена < 5 — построить 6) — все они упомянуты в
`arcomage_plan_deprecated.md`, но ни одной такой карты пока нет в `.tres`.
...
**Описание:** По `arcomage_plan_deprecated.md` должен существовать тип ИИ, приоритизирующий рост собственной башни через
...
**Описание:** Реализовать 3 точки интеграции по `arcomage_plan_deprecated.md` 4.1: «второе дыхание» при поражении,
...
**Описание:** Прямое требование матрицы рисков из `arcomage_roadmap_deprecated.md` («Автоматические тесты боя ИИ против
...
Итого ориентировочно 9–11 недель — совпадает с изначальной оценкой в `arcomage_plan_deprecated.md`, но теперь основано на
...
3. Старые файлы `arcomage_plan_deprecated.md`, `arcomage_roadmap_deprecated.md`, `arcomage_detailed_roadmap_deprecated.md`,
   `arcomage_implementation_detail_deprecated.md` можно оставить как исторический контекст/референс по контенту (списки
   карт, артефактов), но не как источник статуса разработки — эту роль теперь выполняет данный документ.
