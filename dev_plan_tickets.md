...
> ⚠️ Частично реализовано: `.gitignore` почищен (убран задвоенный `/_to_delete/`) и расширен —
> добавлены служебные файлы GUT (`.gut_editor_config.json`, `/gut_results.xml`, см. ARC-072/066) и
> `.DS_Store`/`Thumbs.db`. Экспортные паттерны (`*.exe`, `*.pck`, `*.console.exe`, `/web/`, `/builds/`)
> уже были на месте. Пользовательские сохранения (`user://savegame.json` в `profile_manager.gd`) в
> репозиторий не попадают в принципе — это виртуальный путь Godot вне папки проекта, отдельного
> правила не требуется. Не проверено — нужен реальный полный экспорт в Web и Windows и `git status`
> после него; движка и шаблонов экспорта в песочнице агента нет.
