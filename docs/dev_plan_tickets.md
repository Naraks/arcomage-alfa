...
> `tests/test_battle_screen.gd` — тест на `_compare_cards_for_view()`. Не проверено вживую: полный прогон GUT и
> сам попап глазами в редакторе (открывается ли, закрывается ли по клику вне и по кнопке «Закрыть», не съезжает
> ли центрирование).
