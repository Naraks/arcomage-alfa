...
> кода, та же скрытая ошибка, просто ещё не замеченная (300×200 меньше и смещение было не так заметно). Не
> проверено вживую: сам факт, что оба попапа теперь центрируются корректно.
