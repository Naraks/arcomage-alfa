...
>
> **Не проверено вручную** (нет доступа к редактору Godot из песочницы): реальный визуальный вид экрана, клики по кнопкам в живой сцене, переход туда-обратно между картой мира и Событием на реальном забеге, действительно ли карта/золото/бонус применяются корректно при живом клике (а не только в юнит-тесте).
