...
> ⚠️ Частично реализовано: `world_map_screen.gd` — `_compute_available_nodes()` считает доступные узлы от
...
> некорректный индекс → пустой список без краша). Не проверено вживую: нужно пройти узел на карте в редакторе
> и убедиться, что открываются следующие узлы, а остальные остаются заблокированы.
