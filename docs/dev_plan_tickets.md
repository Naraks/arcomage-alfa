...
>
> **Не проверено вручную** (нет доступа к редактору Godot из песочницы): реальный визуальный вид экрана (лейауты/скролл при большой колоде), клики по кнопкам покупки/удаления в живой сцене, переход туда-обратно между картой мира и магазином на реальном забеге, поведение `_center_control`-подобного позиционирования (здесь не используется — экран на весь Control, не попап).
