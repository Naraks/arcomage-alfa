...
> ⚠️ Частично реализовано: `MatchSettings.run_deck: Array[CardData]` — колода забега, персистентна между
...
> мутируется/не расходуется вызовом `setup_match()`; без `run_deck` работает старый фоллбэк. Известное,
> сознательно не тронутое упрощение: колода общая на игрока и ИИ ("для простоты прототипа", было так и до
> ARC-016) — ИИ тоже тянет карты из `run_deck` игрока; отдельная колода ИИ вне рамок этого тикета.
>
...
> меняется при изменении колоды игрока — независимые массивы, не общая ссылка). Не проверено вживую: полный
> прогон GUT и сам бой (что ИИ по-прежнему нормально играет карты из своей колоды).
