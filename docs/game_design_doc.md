...
### 5.4 Примеры карт (полный каталог — в `cards_catalog.html`)
...
точные `cost`/эффекты и итоговое количество по каждому типу ресурса — **только** в `cards_catalog.html` (ARC-024:
эта таблица уже расходилась с фактическими значениями после балансировки ARC-023, поэтому дублировать здесь
точные цифры и общий счётчик карт больше не будем — обновляйте их только в `cards_catalog.html`).
