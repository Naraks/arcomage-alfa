...
func _score_effect(effect: EffectData, player_actor: PlayerData, player_enemy: PlayerData) -> float:
	...
	if type == EffectType.Type.BUILD_TOWER:
	...
	elif type == EffectType.Type.MOD_QUARRY:
		priority += value * 8.0
	...
	elif type == EffectType.Type.MOD_MAGIC or type == EffectType.Type.MOD_DUNGEON:
		priority += value * 1.0
		...
		type == EffectType.Type.GAIN_RESOURCE
		or type == EffectType.Type.STEAL_RESOURCE
		or type == EffectType.Type.DRAIN_RESOURCE
		or type == EffectType.Type.DRAW_CARD
	):
		priority += value * 1.0
