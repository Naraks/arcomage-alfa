...
func _score_effect(effect: EffectData, player_actor: PlayerData, player_enemy: PlayerData) -> float:
	...
	if type == EffectType.DIRECT_DAMAGE:
	...
	if type == EffectType.GAIN_RESOURCE:
		priority += value * 3.0

	if type == EffectType.STEAL_RESOURCE:
		priority += value * 4.0

	if type == EffectType.DRAIN_RESOURCE:
		priority += value * 2.5

	if type == EffectType.DRAW_CARD:
		priority += value * 3.0
