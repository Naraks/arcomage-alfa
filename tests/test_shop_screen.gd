extends GutTest
## Юнит-тесты чистых хелперов ShopScreen (ARC-012): расчёт цены покупки и
## сортировка карт для отображения в разделе удаления.
##
## shop_screen.gd — обычный Control-скрипт сцены (без class_name), поэтому
## экземпляр создаётся через load().new() и НЕ добавляется в дерево сцены, как
## и в tests/test_battle_screen.gd — тестируемые методы не трогают
## @onready-поля/сцену, только считают/сравнивают переданные CardData.
