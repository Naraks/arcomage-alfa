extends Control
## ARC-042 / UI 04/13 (#99): компактная доступная панель настроек.
## Реальные параметры сгруппированы по назначению; пустые секции не создают
## фиктивных опций. Изменения сразу применяются и сохраняются через
