extends Control
## ARC-013 / UI 08/13 (#103): узел «Отдых» с превью постоянного бонуса.
## Игрок сначала выбирает крупную карточку, затем отдельно подтверждает выбор.
## Бонус действует во всех следующих боях текущего забега; лимиты не дают
## стартовым значениям бесконтрольно расти при повторных посещениях Отдыха.
