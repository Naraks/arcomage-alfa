# Game Design Document — Alfa (Arcomage Roguelike)

> Документ собирается поэтапно. Каждый раздел согласован и добавлен по одному юниту за раз.
> Ссылки на технический бэклог см. в `dev_plan_tickets.md` (тот же проект).

---

## 1. Обзор игры

**Рабочее название:** Alfa
...
1. **Строительная победа:** своя Башня достигла высоты 100.
...
Каждый бой — самодостаточная партия: Башня, Стена, ресурсы и уровни генераторов **всегда сбрасываются к базовым значениям** (20 / 5 / по 5 / уровень 1) в начале боя, в бою на карте мира, в элитном бою, в бою с боссом и в Быстром бою одинаково. Это принципиально, потому что в игре есть условие «построй Башню до 100» — если бы Стена/Башня переносились между боями как «здоровье» (по аналогии с *Slay the Spire*), первая же строительная победа делала бы все следующие бои забега тривиальными.
...
| **Строительство** | Увеличивает свою Стену или Башню | «Малая стена», «Основание», «Укрепление» |
...
| :--- | :--- | :--- | :--- | :--- |
| Каменоломня | Кирпичи | Обычная | 1 | Генератор Кирпичей +1 |
...
| Армагеддон | Гемы | Редкая | 7 | 16 урона Башне врага напрямую |
| Дракон | Звери | Редкая | 5 | 12 урона Башне + Стена врага −5 |

Полный сбалансированный набор из 60 карт (20 на каждый тип ресурса, 8 обычных / 7 необычных / 5 редких) — в `cards_list.md`.
...
### 8.3 Финальный набор артефактов (8 штук, полное описание — в `artifacts_list.md`)
...
Все точки выше реализуются через Yandex Games SDK (`ysdk.adv`, `ysdk.payments`, `ysdk.player`) — детали инициализации и обработки колбэков находятся в техническом бэклоге (`dev_plan_tickets.md`, Эпик H).
...
| **Рендерер** | Compatibility/GLES3, без Forward+ и лишних 3D-модулей | Игра полностью 2D — 3D-физика и продвинутый рендер только раздувают бандл (см. `dev_plan_tickets.md`, ARC-007) |
...
Технические детали реализации (экспорт-пресеты, конкретные тикеты) — в `dev_plan_tickets.md`, Эпики A и H.
